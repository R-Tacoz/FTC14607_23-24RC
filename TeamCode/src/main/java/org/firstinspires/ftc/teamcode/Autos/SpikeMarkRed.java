...
import org.firstinspires.ftc.teamcode.CV.SpikeDetectProcessorRed;
...
@Config
@Autonomous(name = "EOCV Auto - Left Red", group = "main")
...
//        while(opModeIsActive()){
        SpikeDetectProcessorRed.SpikePosition spike = spikeProcessor.getSpikePosition();
        ...
        //zones
        int zone = 0;
        if (spike == SpikeDetectProcessorRed.SpikePosition.LEFT) {
            zone = 1;
        } else if (spike == SpikeDetectProcessorRed.SpikePosition.CENTER) {
            zone = 2;
        } else {
            zone = 3;
        }
